...
def update_ax(arm_points):
    ...
    ax['up'].plot(arm_x[:-1], arm_z[:-1], 'gray')
    ax['up'].plot([arm_x[2], arm_x[4]], [arm_z[2], arm_z[4]], 'gray')
    ax['up'].scatter(arm_x, arm_z, 2, 'gray')

    ax['down'].plot(arm_x[:-1], arm_y[:-1], 'gray')
    ax['down'].plot([arm_x[2], arm_x[4]], [arm_y[2], arm_y[4]], 'gray')
    ax['down'].scatter(arm_x, arm_y, 2, 'gray')


update_ax([np.array([0, 0, 0.8]), np.array([0, 0, 1.85]),
           np.array([0, 0, 2.45]),
           np.array([0, 0, 3.45]), np.array([0, 0, 3.45])])
...
def update(val):
    arm_points = armPoints([shoulder_angle.val/180*pi,
